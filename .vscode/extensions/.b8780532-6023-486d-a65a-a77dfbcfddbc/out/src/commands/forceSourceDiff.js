...
const metadataCacheService_1 = require("../conflict/metadataCacheService");
...
function forceSourceDiff(sourceUri) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (defaultUsernameorAlias) {
            const executor = new metadataCacheService_1.MetadataCacheExecutor(defaultUsernameorAlias, messages_1.nls.localize('force_source_diff_text'), 'force_source_diff', handleCacheResults);
...
function forceSourceFolderDiff(explorerPath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const commandlet = new util_1.SfdxCommandlet(new util_1.SfdxWorkspaceChecker(), new util_1.FilePathGatherer(explorerPath), new metadataCacheService_1.MetadataCacheExecutor(username, 'Source Diff', 'source-diff-loader', handleCacheResults));
        yield commandlet.run();
    });
}
    ...
    return __awaiter(this, void 0, void 0, function* () {
        if (cache) {
            if (cache.selectedType === "individual" /* Individual */ && cache.cache.components) {
                yield conflictDetectionService.diffOneFile(cache.selectedPath, cache.cache.components[0], username);
            }
            else if (cache.selectedType === "folder" /* Folder */) {
                yield conflictDetectionService.diffFolder(cache, username);
            }
        }
        else {
            notifications_1.notificationService.showErrorMessage(messages_1.nls.localize('force_source_diff_components_not_in_org'));
        }
    });
}
